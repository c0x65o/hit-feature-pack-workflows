...
function TaskCard({ task, onApprove, onDeny, }) {
    ...
    return (_jsx(Card, { className: "border-l-4 border-l-amber-500", children: _jsxs("div", { className: "flex items-start justify-between gap-4", children: [_jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center gap-2 mb-2", children: [_jsx(User, { size: 16, className: "text-amber-500" }), _jsx("span", { className: "font-medium", children: task.type === 'approval'
                                        ? 'Approval Required'
                                        : task.type === 'input'
                                            ? 'Human Input Required'
                                            : 'Review Required' }), _jsx(Badge, { variant: task.status === 'open'
                                        ? 'warning'
                                        : task.status === 'approved'
                                            ? 'success'
                                            : task.status === 'denied'
                                                ? 'error'
                                                : 'default', children: task.status })] }), _jsxs("div", { className: "text-sm text-gray-600 dark:text-gray-400 space-y-1", children: [_jsxs("div", { children: [_jsx("span", { className: "text-gray-500", children: "Node:" }), ' ', _jsx("code", { className: "bg-gray-100 dark:bg-gray-800 px-1 rounded text-xs", children: task.nodeId })] }), task.assignedTo && Object.keys(task.assignedTo).length > 0 && (_jsxs("div", { children: [_jsx("span", { className: "text-gray-500", children: "Assigned to:" }), ' ', _jsx("code", { className: "font-mono text-xs bg-gray-100 dark:bg-gray-800 px-1 rounded", children: JSON.stringify(task.assignedTo) })] })), task.prompt && Object.keys(task.prompt).length > 0 && (_jsxs("details", { className: "mt-2", children: [_jsx("summary", { className: "cursor-pointer text-xs text-gray-500 hover:text-gray-700", children: "Show prompt" }), _jsx("div", { className: "mt-2", children: _jsx(JsonBlock, { value: task.prompt }) })] }))] })] }), isPending && (_jsxs("div", { className: "flex gap-2", children: [_jsxs(Button, { variant: "primary", size: "sm", onClick: onApprove, children: [_jsx(ThumbsUp, { size: 14, className: "mr-1" }), "Approve"] }), _jsxs(Button, { variant: "secondary", size: "sm", onClick: onDeny, children: [_jsx(ThumbsDown, { size: 14, className: "mr-1" }), "Deny"] })] }))] }) }));
}
// ─────────────────────────────────────────────────────────────────────────────
// Main component
// ─────────────────────────────────────────────────────────────────────────────
export function WorkflowRunDetail({ runId, onNavigate }) {
    ...
    const eventStats = useMemo(() => {
        ...
        events.forEach((e) => {
            if (e.name.includes('error') || e.name.includes('fail'))
                stats.errors++;
            if (e.name.includes('wait') || e.name.includes('pause') || e.name.includes('waiting'))
                stats.waits++;
        });
        return stats;
    }, [events]);
