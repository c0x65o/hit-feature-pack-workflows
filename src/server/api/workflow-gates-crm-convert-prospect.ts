...
function normalizeEntries(entries: unknown): AclEntry[] {
  ...
  for (const e of entries) {
    ...
    const permissions = Array.isArray(o.permissions) ? o.permissions.map((p: any) => String(p)).filter(Boolean) : [];
    if ((principalType !== 'user' && principalType !== 'group' && principalType !== 'role') || !principalId) continue;
...
async function getWorkflowId(db: any, userId: string): Promise<string> {
  const now = new Date();
  const existing = await db.execute(sql`select id from "workflows" where slug = ${WORKFLOW_SLUG} limit 1`);
  const wid = (existing as any)?.rows?.[0]?.id ?? null;
  ...
  const ins = await db.execute(sql`
    ...
    returning id
  `);
  const newId = (ins as any)?.rows?.[0]?.id ?? null;
  if (!newId) throw new Error('Failed to create workflow');
  return String(newId);
}

async function getLatestPublishedDefinition(db: any, workflowId: string): Promise<{ id: string; version: number; definition: any } | null> {
...
function getGateEntriesFromDefinition(definition: any): AclEntry[] {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const ins = await db.execute(sql`
      ...
      returning id
    `);
    const workflowVersionId = (ins as any)?.rows?.[0]?.id ?? null;
    if (!workflowVersionId) return NextResponse.json({ error: 'Failed to save gate config' }, { status: 500 });
